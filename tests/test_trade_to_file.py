...
def event_handler_order_update(message):
    ...
    if int(message.get('flid', 0)) != 0:
     with open('Tradefile.txt', 'a') as file:
        file.write(str(message)+ '\n')
        fill_id=message.get('flid', 0)
        print("order event:"+f'Fill ID={fill_id} inserted to Tradefile.txt')
...
#yaml for parameters
with open('/home/ubuntu/workspace/testing/kambalatech_oct4/NorenApi-Py/cred.yml') as f:
